...
export async function GET(request: Request) {
  ...
  try {
    return Response.json({ members: getFamilySummaries(getAccountId(request), lang) });
  } catch (err) {
...
export async function POST(request: Request) {
  try {
    ...
    const member = addPatient(accountId, name, relation, age);
